...
class UnetDataset(Dataset):
            ...
            for line in lines:
                ...
                for single_img in imgs:
                    if single_img[:-4] in modals:
                        ...
                        roi_data = sitk.GetArrayFromImage(roi_img)
                        roi_tensor = torch.from_numpy(roi_data / 1.0)
                        # roi_tensor = interpolate(roi_tensor, size=[224], mode='nearest')
    ...
    def select_roi(self, number = 1):
        ...
        for roi_img, data_img in zip(self.roi_data, self.fin_data):
            ...
            # print(c)
            max_slice = torch.index_select(data_img, 0, indices).type(torch.cuda.FloatTensor)
            roi_slice = torch.index_select(roi_img, 0, indices).type(torch.cuda.LongTensor)
            fin_data.append(max_slice)
            roi_data.append(roi_slice)
        self.fin_data = fin_data
        self.roi_data = roi_data


class UnetDataLoader(BaseDataLoader):
    def __init__(self, data_dir, modals, batch_size, shuffle=True, validation_split=0.0, num_workers=1,
                 use_roi=False, transform=None):
        self.data_dir = data_dir
        self.dataset = UnetDataset(self.data_dir, modals, use_roi, transform)
        self.dataset.select_roi(10)
        super().__init__(self.dataset, batch_size, shuffle, validation_split, num_workers)
