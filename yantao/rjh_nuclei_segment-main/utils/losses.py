...
class MulticlassDiceLoss3D(nn.Module):
    ...
    def forward(self, input, target, c, weights=None):
        ...
        print(input.shape, target_to_one_hot.shape)
...
def total_loss_5ROI(pred, target, c, weights=[0.3, 0.7]):
    """
    ...
    roi_names = ['CN_dice', 'GP_dice', 'PUT_dice', 'RN_dice', 'SN_dice']
    for i in range(len(roi_names)):
        loss_dict[roi_names[i]] = dice_loss_list[i]
...
def total_loss_6ROI(pred, target, c, weights=[0.3, 0.7]):
    """
    	a list of input tesor of shape = (N, C, H, W)
    	target tensor of shape = (N, 1, H, W)
    """
    loss_dict = {}
    dice_loss_list, loss_dict['dice_loss'] = MulticlassDiceLoss3D()(pred, target, c)
    loss_dict['ce_loss'] = MulticlassEntropyLoss3D()(pred, target, c)
    loss_dict['total_loss'] = loss_dict['dice_loss']*weights[0] + loss_dict['ce_loss']*weights[1]

    for i in range(c-1):
        loss_dict[f'roi{i+1}'] = dice_loss_list[i]
