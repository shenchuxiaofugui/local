...
parameters = dict(
        max_iteration=40000,   #which means the maximum of iteration
        spshot=30,               #which means saving checkpoint every 20000 iterations
        nclass=2,              #which means binary tasks. i.e. model output
        b_size=8,
        sshow=655,             #which means showing the training loss every 20 iterations
        phase="train",                   # train or test
        param=False,                     # Loading checkpoint
        dataset="Magrabia",              # test or val (dataset)
        snap_num=20,                     # Snapshot Number
        gpu_ids='0,1,2,3',                     # CUDA_VISIBLE_DEVICES
        train_dataroot=r'D:\python\learn\MRNet-main\MRNet_code\DiscRegion',
        test_dataroot=r'D:\python\learn\MRNet-main\MRNet_code\DiscRegion',
)
